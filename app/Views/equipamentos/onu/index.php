...
<script>
    const tableOnu = document.getElementById('tableOnu');
    ...
    const renderTableTwo = (data) => {
        onuCount.innerText = data.length

        const tbody = document.querySelector('tbody')
        tbody.innerHTML = ''
        data.forEach(onu => {

            Object.keys(onu).forEach(key => {
                if (onu[key] === null) {
                    onu[key] = ''
                }
            });
                ...
                <td class='text-end'>
                    <div class="dropstart">
                        ...
                        <ul class="dropdown-menu text-center me-2" style="background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(7px); border-radius: 15px; border: 2px solid #198754;">
                            <li class="dropdown-item" onclick="editOnu(${onu.id})">Editar</li>
                            <li class="dropdown-item" onclick="deleteOnu(${onu.id})">Excluir</li>
                        </ul>
                    </div>
                </td>
            `
